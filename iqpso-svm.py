...
class QPSO(object):
    ...
    ### 2.3 粒子位置更新
    def updata(self, particle_loc, gbest_parameter, pbest_parameters, current_iter_num):
        '''粒子位置更新
        input:self(object):QPSO类
              particle_loc(list):粒子群位置列表
    ...
    def plot(self, average_results, best_results):
        ...
        plt.title('IQPSO_RBF_SVM parameter optimization')
        plt.show()
    ...
    def main(self):
        ...
        for i in range(self.iter_num):
            ...
            ### 3.4 更新粒子群
            particle_loc = self.updata(particle_loc, gbest_parameter, pbest_parameters, i + 1)
        ## 4.结果展示
        # results.sort()
